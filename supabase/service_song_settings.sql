...
create table if not exists public.service_song_settings (
  ...
  constraint service_song_settings_item_fkey foreign key (service_id, service_item_id)
...
create index if not exists service_song_settings_service_id_idx
on public.service_song_settings(service_id);
...
create policy "Public can read service song settings"
on public.service_song_settings for select to public using (true);
...
create or replace function public.set_service_song_key_override(
...
begin
  ...
  if p_service_id is null or p_service_item_id is null or p_song_id is null then
    raise exception 'Service, service item, and song are required';
  end if;
  ...
  if target_status not in ('active', 'planned') then
    raise exception 'Only active or planned services can be edited';
  end if;
  ...
  if not exists (select 1 from public.songs song where song.id = p_song_id) then
    raise exception 'Song not found';
  end if;

  if p_key_override is null then
    delete from public.service_song_settings setting
    where setting.service_item_id = p_service_item_id and setting.song_id = p_song_id;
    return;
  end if;
  ...
  values (p_service_id, p_service_item_id, p_song_id, normalized_key)
  on conflict (service_item_id, song_id) do update
  set key_override = excluded.key_override, updated_at = now();
end; $$;

revoke all on function public.set_service_song_key_override(smallint, uuid, uuid, text)
from public, anon, authenticated;
grant execute on function public.set_service_song_key_override(smallint, uuid, uuid, text)
to authenticated;
