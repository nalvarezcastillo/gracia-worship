...
begin
  ...
  if new.current_song_id is not null then
    if target_item.type <> 'worship' or not exists (
      select 1 from jsonb_array_elements(coalesce(target_item.song_ids, '[]'::jsonb)) as song_entry
      where case jsonb_typeof(song_entry)
        when 'string' then song_entry #>> '{}'
        when 'object' then coalesce(song_entry ->> 'songId', song_entry ->> 'song_id', song_entry ->> 'id')
        else null end = new.current_song_id::text
...
declare target_item public.service_items%rowtype; song_entry jsonb; raw_seconds text; raw_minutes text; library_duration text;
begin
  ...
  select entry into song_entry from jsonb_array_elements(coalesce(target_item.song_ids, '[]'::jsonb)) as entry
  where case jsonb_typeof(entry) when 'string' then entry #>> '{}' when 'object' then coalesce(entry ->> 'songId', entry ->> 'song_id', entry ->> 'id') else null end = p_song_id::text limit 1;
  if song_entry is null then raise exception 'The song does not belong to the selected worship block'; end if;
