...
create or replace function public.change_service_item_song(p_service_id smallint, p_service_item_id uuid, p_song_id uuid)
returns void language plpgsql security definer set search_path = pg_catalog, public as $$
declare target_status text; target_item public.service_items%rowtype; replacement_title text;
begin
  if auth.uid() is null then raise exception 'Debes iniciar sesión para cambiar una canción.'; end if;
  if p_service_id is null or p_service_item_id is null or p_song_id is null then raise exception 'El servicio, el elemento y la canción son obligatorios.'; end if;
  perform pg_advisory_xact_lock(71831, p_service_id::integer);
  select status into target_status from public.active_setlist where id = p_service_id for update;
  if not found then raise exception 'No se encontró el servicio.'; end if;
  if target_status not in ('planned', 'active') then raise exception 'Solo se pueden cambiar canciones de servicios planificados o activos.'; end if;
  select * into target_item from public.service_items where service_id = p_service_id and id = p_service_item_id for update;
  if not found then raise exception 'El elemento no pertenece a este servicio.'; end if;
  if target_item.type <> 'song' then raise exception 'Solo se pueden cambiar elementos de tipo canción.'; end if;
  select title into replacement_title from public.songs where id = p_song_id;
  if not found then raise exception 'No se encontró la canción seleccionada.'; end if;
  if target_item.song_id = p_song_id then return; end if;
  if exists (select 1 from public.service_item_runs where service_id = p_service_id and service_item_id = p_service_item_id) then raise exception 'No se puede cambiar una canción que ya participó en En Vivo.'; end if;
  if exists (select 1 from public.live_service_state where service_id = p_service_id and current_item_id = p_service_item_id) then raise exception 'No se puede cambiar la canción que está seleccionada en En Vivo.'; end if;
  delete from public.service_song_settings where service_id = p_service_id and service_item_id = p_service_item_id;
  update public.service_items set song_id = p_song_id, title = replacement_title, planned_duration_seconds = null where service_id = p_service_id and id = p_service_item_id;
end; $$;
revoke all on function public.change_service_item_song(smallint, uuid, uuid) from public, anon, authenticated;
grant execute on function public.change_service_item_song(smallint, uuid, uuid) to authenticated;
...
create table if not exists public.service_item_responsibilities (
  ...
  sort_order integer not null default 0 check (sort_order >= 0),
  created_at timestamptz not null default now(),
  constraint service_item_responsibilities_item_fkey foreign key (service_id, service_item_id)
    references public.service_items(service_id, id) on delete cascade,
  constraint service_item_responsibilities_assignment_fkey foreign key (service_id, service_team_assignment_id)
    references public.service_team_assignments(service_id, id),
  constraint service_item_responsibilities_item_assignment_key unique (service_item_id, service_team_assignment_id)
);
...
create policy "Public can read service item responsibilities" on public.service_item_responsibilities
for select to anon, authenticated using (true);
...
create or replace function public.set_service_item_responsibilities(p_service_id smallint, p_service_item_id uuid, p_assignment_ids uuid[])
returns void language plpgsql security definer set search_path = pg_catalog, public as $$
declare target_status text; requested_ids uuid[] := coalesce(p_assignment_ids, '{}'::uuid[]);
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
  if p_service_id is null or p_service_item_id is null then raise exception 'Service and service item are required'; end if;
  perform pg_advisory_xact_lock(71835, p_service_id::integer);
  select status into target_status from public.active_setlist where id = p_service_id for update;
  if not found then raise exception 'No se encontró el servicio.'; end if;
  if target_status not in ('planned', 'active') then raise exception 'Solo se pueden editar responsabilidades de servicios planificados o activos.'; end if;
  perform 1 from public.service_items where service_id = p_service_id and id = p_service_item_id for update;
  if not found then raise exception 'El elemento no pertenece a este servicio.'; end if;
  if cardinality(requested_ids) <> (select count(distinct assignment_id) from unnest(requested_ids) requested(assignment_id)) then raise exception 'No se permiten responsables duplicados.'; end if;
  if exists (select 1 from unnest(requested_ids) requested(assignment_id) left join public.service_team_assignments assignment on assignment.service_id = p_service_id and assignment.id = requested.assignment_id where assignment.id is null) then raise exception 'Todas las asignaciones deben pertenecer al equipo de este servicio.'; end if;
  delete from public.service_item_responsibilities where service_id = p_service_id and service_item_id = p_service_item_id;
  insert into public.service_item_responsibilities(service_id, service_item_id, service_team_assignment_id, sort_order)
  select p_service_id, p_service_item_id, assignment_id, ordinality - 1 from unnest(requested_ids) with ordinality requested(assignment_id, ordinality) order by ordinality;
end; $$;
revoke all on function public.set_service_item_responsibilities(smallint, uuid, uuid[]) from public, anon, authenticated;
grant execute on function public.set_service_item_responsibilities(smallint, uuid, uuid[]) to authenticated;

create or replace function public.copy_service_team(p_source_service_id smallint, p_target_service_id smallint)
returns jsonb language plpgsql security definer set search_path = pg_catalog, public as $$
declare target_status text; source_assignment record; new_assignment_id uuid; assignment_count integer; people_count integer; resource_count integer;
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
  if p_source_service_id is null or p_target_service_id is null then raise exception 'Source and target services are required'; end if;
  if p_source_service_id = p_target_service_id then raise exception 'El servicio de origen y el servicio actual deben ser diferentes.'; end if;
  perform pg_advisory_xact_lock(71834, p_target_service_id::integer);
  select status into target_status from public.active_setlist where id = p_target_service_id for update;
  if not found then raise exception 'No se encontró el servicio actual.'; end if;
  if target_status not in ('planned', 'active') then raise exception 'Solo se puede copiar equipo a un servicio planificado o activo.'; end if;
  if not exists (select 1 from public.active_setlist where id = p_source_service_id) then raise exception 'No se encontró el servicio de origen.'; end if;
  if exists (select 1 from public.service_team_assignments where service_id = p_target_service_id) then raise exception 'Este servicio ya tiene un equipo asignado.'; end if;
  if not exists (select 1 from public.service_team_assignments where service_id = p_source_service_id) then raise exception 'El servicio de origen no tiene asignaciones de equipo.'; end if;
  if exists (select 1 from public.service_team_assignment_resources l left join public.resources r on r.id = l.resource_id where l.service_id = p_source_service_id and r.id is null) then raise exception 'El equipo de origen contiene un recurso que ya no existe.'; end if;
  if exists (select 1 from public.service_team_assignment_resources l join public.resources r on r.id = l.resource_id where l.service_id = p_source_service_id and r.active is not true) then raise exception 'El equipo de origen contiene un recurso inactivo.'; end if;
  if exists (select 1 from public.service_team_assignment_resources s join public.service_team_assignment_resources t on t.service_id = p_target_service_id and t.resource_id = s.resource_id where s.service_id = p_source_service_id) then raise exception 'Un recurso del equipo de origen ya está asignado en el servicio actual.'; end if;
  select count(*), count(distinct lower(btrim(person_name))) into assignment_count, people_count from public.service_team_assignments where service_id = p_source_service_id;
  select count(*) into resource_count from public.service_team_assignment_resources where service_id = p_source_service_id;
  for source_assignment in select * from public.service_team_assignments where service_id = p_source_service_id order by sort_order, created_at, id loop
    ...
    values (new_assignment_id, p_target_service_id, source_assignment.team_member_id, source_assignment.person_name, source_assignment.role_name, source_assignment.microphone_name, source_assignment.sort_order, now(), now());
    insert into public.service_team_assignment_resources(id, service_id, assignment_id, resource_id, created_at)
    select gen_random_uuid(), p_target_service_id, new_assignment_id, resource_id, now() from public.service_team_assignment_resources
    where service_id = p_source_service_id and assignment_id = source_assignment.id order by created_at, id;
  end loop;
  return jsonb_build_object('source_service_id', p_source_service_id, 'target_service_id', p_target_service_id, 'people_copied', people_count, 'assignments_copied', assignment_count, 'resources_copied', resource_count);
end; $$;
revoke all on function public.copy_service_team(smallint, smallint) from public, anon, authenticated;
grant execute on function public.copy_service_team(smallint, smallint) to authenticated;
