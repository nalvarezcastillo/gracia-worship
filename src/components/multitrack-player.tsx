    ...
    <MusicalGridProvider value={musicalGrid}><SongSectionsProvider value={sections}><div data-multitrack-layout={layout}>
      ...
      <div className="flex items-center gap-3 sm:gap-5 lg:hidden">
        ...
        <div className="min-w-0 flex-1">
          {currentMusicalPosition ? <div className="mb-2 flex justify-end"><MusicalPositionDisplay compact position={currentMusicalPosition} /></div> : null}
          {layout === "playback" ? <AudioWaveformTimeline buffers={waveformBuffers} currentTime={currentTime} duration={duration} loopSectionId={loopSectionId} onSeek={(value) => void seek(value)} /> : <input type="range" min={0} max={duration || 0} step="0.1" value={Math.min(currentTime, duration)} onChange={(event) => void seek(Number(event.target.value))} aria-label={`Posición del multitrack de ${title}`} className="audio-progress w-full" style={{ "--audio-progress": `${duration > 0 ? (currentTime / duration) * 100 : 0}%` } as React.CSSProperties} />}
          {layout === "playback" && orderedSections.length ? <SectionNavigation compact currentSection={currentSection} loopSection={loopRegion?.section ?? null} nextTarget={nextSectionTarget} previousTarget={previousSectionTarget} onSeek={(value) => void seek(value)} onToggleLoop={toggleSectionLoop} /> : null}
