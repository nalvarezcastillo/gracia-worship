...
export function MultitrackPlayer({ active = true, artist, artworkUrl, bpm, canEditServiceMix = false, canNext = false, canPrevious = false, effectiveKey, externalStopSignal = 0, grid = null, initialStemMixes = EMPTY_PERSISTED_STEM_MIXES, layout = "playback", onEngineChange, onNext, onPrevious, serviceId, serviceItemId, showStop = false, songId, stems, timeSignature, title }: { active?: boolean; artist?: string | null; artworkUrl?: string | null; bpm?: number | null; canEditServiceMix?: boolean; canNext?: boolean; canPrevious?: boolean; effectiveKey?: string | null; externalStopSignal?: number; grid?: MusicalGrid | null; initialStemMixes?: PersistedStemMix[]; layout?: "playback" | "song-detail"; onEngineChange?: (engine: PlaybackEngine | null) => void; onNext?: () => void; onPrevious?: () => void; serviceId?: number; serviceItemId?: string; showStop?: boolean; songId?: string; stems: PublicSongStem[]; timeSignature?: string | null; title: string }) {
  ...
  const getAuthoringPosition = useCallback(() => {
    const engine = engineRef.current;
    const position = engine?.isPlaying ? getPlaybackTime(engine.context.currentTime) : currentTime;
    return Math.min(duration, Math.max(0, position));
  }, [currentTime, duration, getPlaybackTime]);
  ...
  return (
    <MusicalGridProvider value={musicalGrid}><SongSectionsProvider value={sections}><div data-multitrack-layout={layout}>
      ...
      {layout === "song-detail" && showPlaybackAuthoring && stems[0]?.song_key_id ? <div role="presentation" className="fixed inset-0 z-[70] flex items-end justify-center bg-black/75 backdrop-blur-sm sm:items-center sm:p-5"><section role="dialog" aria-modal="true" aria-labelledby="playback-authoring-title" className="flex max-h-[calc(100dvh-env(safe-area-inset-top))] w-full max-w-4xl flex-col rounded-t-3xl border border-white/10 bg-zinc-950 shadow-2xl sm:max-h-[92dvh] sm:rounded-2xl"><header className="flex items-center justify-between border-b border-white/[0.07] px-4 py-3 sm:px-5"><div><p className="text-[0.625rem] font-bold uppercase tracking-[0.15em] text-emerald-400">Arreglo</p><h2 id="playback-authoring-title" className="mt-0.5 text-lg font-bold text-white">Configurar Playback</h2></div><button type="button" onClick={() => setShowPlaybackAuthoring(false)} aria-label="Cerrar configuración de Playback" className="grid size-11 place-items-center rounded-lg text-xl text-zinc-500 hover:bg-white/[0.05] hover:text-white">×</button></header><div className="min-h-0 flex-1 overflow-y-auto pb-[env(safe-area-inset-bottom)]"><CompactAuthoringTransport currentTime={currentTime} disabled={(status === "partial" && !partialAccepted) || (durationWarnings.length > 0 && !durationAccepted) || contextState === "closed"} duration={duration} isPlaying={isPlaying} onSeek={(value) => void seek(value)} onToggle={() => void togglePlayback()} /><MusicalGridEditor getCurrentTime={getAuthoringPosition} fallbackBpm={bpm ?? null} fallbackTimeSignature={timeSignature ?? null} grid={musicalGrid} onSaved={setMusicalGrid} songKeyId={stems[0].song_key_id} /><section className="border-b border-white/[0.07] bg-white/[0.015]"><div className="flex items-center justify-between gap-3 px-4 py-3"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-zinc-500">Secciones</p><p className="mt-0.5 text-[0.6875rem] text-zinc-600">Escucha, pausa y marca cada límite musical</p></div><button type="button" onClick={openNewSection} className="min-h-9 rounded-lg border border-emerald-400/25 px-3 text-xs font-semibold text-emerald-300">+ Agregar sección</button></div>{orderedSections.length ? <div className="divide-y divide-white/[0.06] border-t border-white/[0.06]">{orderedSections.map((section) => <div key={section.id} className="grid grid-cols-[4.5rem_minmax(0,1fr)_auto] items-center gap-2 px-4 py-2"><span className="font-mono text-xs text-zinc-500">{formatSectionTimestamp(section.start_seconds)}</span><div className="min-w-0"><p className="truncate text-sm font-semibold text-zinc-200">{section.label}</p><p className="text-[0.6875rem] text-zinc-600">{sectionTypeLabel(section.section_type)}{musicalGrid ? ` · ${sectionMusicalLabel(section, musicalGrid)}` : ""}</p></div><div className="flex items-center gap-1"><button type="button" onClick={() => openSectionEditor(section)} aria-label={`Editar sección ${section.label}`} className="min-h-9 rounded-md px-2 text-xs font-semibold text-zinc-400 hover:bg-white/[0.05] hover:text-white">Editar</button><button type="button" onClick={() => void deleteSection(section)} aria-label={`Eliminar sección ${section.label}`} className="min-h-9 rounded-md px-2 text-xs font-semibold text-rose-300/75 hover:bg-rose-400/[0.07] hover:text-rose-200">Eliminar</button></div></div>)}</div> : <p className="border-t border-white/[0.06] px-4 py-5 text-sm text-zinc-600">Esta tonalidad todavía no tiene secciones.</p>}</section></div></section></div> : null}
      {sectionEditor ? <SectionEditorDialog currentTime={currentTime} duration={duration} editor={sectionEditor} error={sectionEditorError} getCurrentTime={getAuthoringPosition} isPlaying={isPlaying} saving={savingSection} transportDisabled={(status === "partial" && !partialAccepted) || (durationWarnings.length > 0 && !durationAccepted) || contextState === "closed"} onChange={setSectionEditor} onClose={() => { if (!savingSection) setSectionEditor(null); }} onSave={() => void saveSection()} onSeek={(value) => void seek(value)} onToggle={() => void togglePlayback()} /> : null}
...
function CompactAuthoringTransport({ currentTime, disabled, duration, isPlaying, onSeek, onToggle }: { currentTime: number; disabled: boolean; duration: number; isPlaying: boolean; onSeek: (time: number) => void; onToggle: () => void }) {
  const progress = duration > 0 ? Math.min(100, Math.max(0, (currentTime / duration) * 100)) : 0;
  return <section aria-label="Transporte de Playback" className="border-b border-white/[0.07] bg-black/20 px-4 py-4 sm:px-5"><p className="text-[0.6875rem] font-bold uppercase tracking-[0.16em] text-zinc-500">Playback</p><div className="mt-3 flex items-center gap-3 sm:gap-4"><button type="button" onClick={onToggle} disabled={disabled || duration <= 0} aria-label={isPlaying ? "Pausar Playback" : "Reproducir Playback"} className="grid size-11 shrink-0 place-items-center rounded-full bg-emerald-400 text-zinc-950 hover:bg-emerald-300 disabled:cursor-not-allowed disabled:opacity-35">{isPlaying ? <PauseIcon /> : <PlayIcon />}</button><input type="range" min={0} max={duration || 0} step={0.001} value={Math.min(currentTime, duration || 0)} onChange={(event) => onSeek(Number(event.target.value))} disabled={disabled || duration <= 0} aria-label="Posición del Playback" className="h-11 min-w-0 flex-1 accent-emerald-400" style={{ "--audio-progress": `${progress}%` } as React.CSSProperties} /></div><div className="mt-1 flex justify-between font-mono text-xs tabular-nums text-zinc-400"><span>{formatSectionTimestamp(currentTime)}</span><span>{formatSectionTimestamp(duration)}</span></div></section>;
}
...
function SectionEditorDialog({ currentTime, duration, editor, error, getCurrentTime, isPlaying, onChange, onClose, onSave, onSeek, onToggle, saving, transportDisabled }: { currentTime: number; duration: number; editor: SectionEditorState; error: string | null; getCurrentTime: () => number; isPlaying: boolean; onChange: (editor: SectionEditorState) => void; onClose: () => void; onSave: () => void; onSeek: (time: number) => void; onToggle: () => void; saving: boolean; transportDisabled: boolean }) {
