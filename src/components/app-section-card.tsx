...
export function AppSectionCard({ accent = true, children, className = "", eyebrow, subtitle, title }: AppSectionCardProps) {
  return (
    <section className={`mt-6 overflow-hidden rounded-3xl border border-white/[0.07] ${accent ? "border-t-[4px] border-t-emerald-500 hover:border-t-emerald-400" : ""} bg-zinc-900/60 shadow-xl shadow-black/10 transition-[border-color,box-shadow] duration-200 hover:border-white/[0.11] hover:shadow-2xl hover:shadow-black/15 sm:mt-8 ${className}`}>
      <div className="border-b border-white/[0.06] bg-gradient-to-br from-emerald-400/[0.07] to-transparent px-5 py-4 sm:px-6 sm:py-5">
        {eyebrow ? <p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-400">{eyebrow}</p> : null}
        <h2 className={`${eyebrow ? "mt-2" : ""} text-2xl font-bold tracking-tight text-white`}>{title}</h2>
