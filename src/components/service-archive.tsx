...
export function ServiceArchive({ services }: { services: ArchivedServiceSummary[] }) {
  ...
  return <>
    ...
    {!services.length ? <p className="mt-6 border-y border-white/[0.07] py-12 text-center text-sm text-zinc-500">No hay servicios archivados.</p> : filteredServices.length ? <div className="mt-6 divide-y divide-white/[0.07] border-y border-white/[0.07]">{filteredServices.map((service) => <article key={service.id} className="py-4"><h2 className="font-semibold text-white">{service.service_name}</h2><p className="mt-1 text-sm text-zinc-400">{formatArchiveDate(service.service_date)}</p><p className="mt-1 text-sm text-zinc-500">{service.itemCount} {service.itemCount === 1 ? "elemento" : "elementos"}</p><div className="mt-3 flex flex-wrap gap-2"><Link href={`/service/${service.id}`} className={actionStyles}>Ver</Link><Link href={`/service/${service.id}/report`} className={actionStyles}>Ver reporte</Link><button type="button" onClick={() => setConfirmation({ id: service.id, kind: "restore", title: service.service_name })} className={actionStyles}>Restaurar como servicio actual</button><button type="button" onClick={() => setConfirmation({ id: service.id, kind: "delete", title: service.service_name })} className={`${actionStyles} text-rose-300`}>Eliminar</button></div></article>)}</div> : <p className="mt-6 py-10 text-center text-sm text-zinc-500">No se encontraron servicios.</p>}
