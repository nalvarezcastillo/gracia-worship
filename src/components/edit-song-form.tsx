...
export function EditSongForm({ song }: { song: SongRecord }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (error) {
        throw new Error("No se pudieron actualizar los datos de la canción.");
      }
      ...
      setIsSaving(false);
      setIsError(true);
      setMessage(error instanceof Error ? error.message : "No se pudo actualizar la canción. Intenta nuevamente.");
    }
  }
    ...
    try {
      ...
      if (paths.length > 0) {
        ...
        if (error) {
          ...
          throw new Error("No se pudieron eliminar los archivos de la canción.");
        }
      }
      ...
      setIsError(true);
      setMessage(error instanceof Error ? error.message : "No se pudo eliminar la canción. Intenta nuevamente.");
    }
  }
    ...
    <form onSubmit={handleSubmit} className="mt-8 space-y-7 sm:mt-10 sm:space-y-8">
      <FormSection title="Datos de la canción">
        <div className="grid gap-5 sm:grid-cols-2 sm:gap-6">
          <TextField label="Título" name="title" defaultValue={song.title} className="sm:col-span-2" />
          <TextField label="Artista" name="artist" defaultValue={song.artist} className="sm:col-span-2" />
          ...
          <TextField label="Duración" name="duration" defaultValue={song.duration} className="sm:col-span-2" />
        </div>
      </FormSection>
      ...
      <div>
        <PrimaryButton type="submit" disabled={isSaving || isDeleting} className="min-h-14 w-full">{isSaving ? "Guardando..." : "Guardar cambios"}</PrimaryButton>
        <button
          type="button"
        ...
        >
          {isDeleting ? "Eliminando..." : "Eliminar canción"}
        </button>
      ...
      {isDeleteDialogOpen ? (
        <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/70 p-3 backdrop-blur-sm sm:grid sm:place-items-center sm:px-4" role="presentation">
          <section role="dialog" aria-modal="true" aria-labelledby="delete-song-title" aria-describedby="delete-song-description" className="max-h-[calc(100dvh-1.5rem)] w-full max-w-md overflow-y-auto rounded-2xl border border-white/10 bg-zinc-900 p-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] shadow-2xl shadow-black/60 sm:rounded-3xl sm:p-7">
            <h2 id="delete-song-title" className="text-2xl font-bold tracking-tight text-white">Eliminar canción</h2>
            <div id="delete-song-description" className="mt-4 space-y-3 text-base leading-7 text-zinc-400">
              <p>¿Seguro que deseas eliminar esta canción permanentemente?</p>
              <p className="text-sm text-rose-300">Esta acción no se puede deshacer.</p>
            </div>
            <div className="mt-7 grid grid-cols-2 gap-3">
              <button type="button" onClick={() => setIsDeleteDialogOpen(false)} disabled={isDeleting} className="min-h-12 rounded-full border border-white/10 bg-white/[0.055] px-5 font-semibold text-white transition-all duration-200 hover:bg-white/10 active:scale-[0.98] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white disabled:opacity-50">Cancelar</button>
              <button type="button" onClick={handleDelete} disabled={isDeleting} className="min-h-12 rounded-full bg-rose-500 px-5 font-semibold text-white shadow-lg shadow-rose-950/30 transition-all duration-200 hover:bg-rose-400 active:scale-[0.98] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-rose-400 disabled:opacity-50">{isDeleting ? "Eliminando..." : "Eliminar"}</button>
            </div>
          </section>
        </div>
      ) : null}
