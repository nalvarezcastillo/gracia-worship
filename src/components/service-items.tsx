  ...
  return (
    <div className="space-y-4 lg:grid lg:grid-cols-[260px_minmax(0,1fr)] lg:gap-0 lg:space-y-0">
      <aside className="hidden border-r border-white/[0.07] bg-zinc-950/35 p-6 lg:block">
        ...
        <nav aria-label="Secciones del servicio" className="mt-7 space-y-1 text-sm font-medium"><a href="#orden" className="block rounded-lg bg-emerald-400/[0.09] px-3 py-2.5 text-emerald-300">Orden</a><Link href={`/service/${serviceId}/preflight`} className="block rounded-lg px-3 py-2.5 text-zinc-400 hover:bg-white/[0.04] hover:text-white">Preparación</Link><Link href={`/admin/service-team?service=${serviceId}`} className="block rounded-lg px-3 py-2.5 text-zinc-400 hover:bg-white/[0.04] hover:text-white">Equipo</Link><Link href={`/admin/resources?service=${serviceId}`} className="block rounded-lg px-3 py-2.5 text-zinc-400 hover:bg-white/[0.04] hover:text-white">Recursos</Link><Link href={`/service/${serviceId}/notes`} className="block rounded-lg px-3 py-2.5 text-zinc-400 hover:bg-white/[0.04] hover:text-white">Notas</Link><Link href={`/service/${serviceId}/rehearsal`} className="block rounded-lg px-3 py-2.5 text-zinc-400 hover:bg-white/[0.04] hover:text-white">Ensayo</Link><Link href={`/service/${serviceId}/playback`} className="block rounded-lg px-3 py-2.5 text-zinc-400 hover:bg-white/[0.04] hover:text-white">Playback</Link><Link href={`/service/${serviceId}/report`} className="block rounded-lg px-3 py-2.5 text-zinc-400 hover:bg-white/[0.04] hover:text-white">Reporte</Link></nav>
      ...
      <nav aria-label="Secciones del servicio" className="-mx-4 flex h-11 gap-0.5 overflow-x-auto border-b border-white/[0.07] px-4 text-[0.8125rem] font-semibold [scrollbar-width:none] lg:hidden [&::-webkit-scrollbar]:hidden"><a href="#orden" aria-current="page" className="flex min-h-11 shrink-0 items-center border-b-2 border-emerald-400 px-2.5 text-emerald-300">Orden</a><Link href={`/service/${serviceId}/preflight`} className="flex min-h-11 shrink-0 items-center border-b-2 border-transparent px-2.5 text-zinc-400">Preparación</Link>{isAdmin ? <><Link href={`/admin/service-team?service=${serviceId}`} className="flex min-h-11 shrink-0 items-center border-b-2 border-transparent px-2.5 text-zinc-400">Equipo</Link><Link href={`/admin/resources?service=${serviceId}`} className="flex min-h-11 shrink-0 items-center border-b-2 border-transparent px-2.5 text-zinc-400">Recursos</Link></> : null}<Link href={`/service/${serviceId}/notes`} className="flex min-h-11 shrink-0 items-center border-b-2 border-transparent px-2.5 text-zinc-400">Notas</Link><Link href={`/service/${serviceId}/rehearsal`} className="flex min-h-11 shrink-0 items-center border-b-2 border-transparent px-2.5 text-zinc-400">Ensayo</Link><Link href={`/service/${serviceId}/playback`} className="flex min-h-11 shrink-0 items-center border-b-2 border-transparent px-2.5 text-zinc-400">Playback</Link><Link href={`/service/${serviceId}/report`} className="flex min-h-11 shrink-0 items-center border-b-2 border-transparent px-2.5 text-zinc-400">Reporte</Link></nav>
            ...
            >
              <div className={`grid grid-cols-[4rem_minmax(0,1fr)_auto] items-start gap-x-1.5 lg:grid-cols-[112px_76px_minmax(0,1fr)_minmax(110px,0.42fr)_72px_auto_auto] lg:items-center lg:gap-x-3 ${item.type === "worship" ? "min-h-7 lg:min-h-9" : "min-h-[3.75rem] lg:min-h-14"}`}>
                ...
                <div className={`min-w-0 ${item.type === "worship" ? "col-span-2 col-start-1 lg:col-span-1 lg:col-start-3" : "col-start-2 lg:col-start-3"}`}>
                  ...
                  {item.type === "text" && item.details && !hasRelationalResponsibility ? <p className="mt-0.5 hidden truncate whitespace-nowrap text-sm font-normal leading-5 text-zinc-500 lg:block">{item.details}</p> : null}
...
function DirectSongItemMetadata({ hasRelationalResponsibility, item, songs }: { hasRelationalResponsibility: boolean; item: ServiceItem; songs: ServiceSong[] }) {
  ...
  const metadata = [song.bpm ? `${song.bpm} BPM` : null, song.time_signature, ...(!hasRelationalResponsibility ? [assignment.name, assignment.role] : [])].filter((value): value is string => Boolean(value));
