...
export function AppSettingsForm({ initialSettings, version }: { initialSettings: AppSettings; version: string }) {
  ...
  return <form onSubmit={save} className="mt-6 grid gap-5 lg:grid-cols-2">
    ...
    <AppActionBar className="lg:col-span-2" separated><PrimaryButton type="submit" disabled={isSaving || !settings.church_name.trim() || !settings.ministry_name.trim() || !settings.service_day.trim() || !settings.service_time.trim()}>{isSaving ? "Guardando..." : "Guardar cambios"}</PrimaryButton><p role="status" aria-live="polite" className={`min-h-5 text-sm ${isError ? "text-rose-400" : "text-emerald-400"}`}>{message}</p></AppActionBar>
  </form>;
}
