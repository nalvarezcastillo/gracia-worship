...
export function AppSettingsForm({ initialSettings, version }: { initialSettings: AppSettings; version: string }) {
  ...
  return <form onSubmit={save} className="mt-6 space-y-6">
    <SettingsSection title="Identidad"><Field label="Nombre de la iglesia"><input required value={settings.church_name} onChange={(event) => setSettings({ ...settings, church_name: event.target.value })} className={inputStyles} /></Field><Field label="Nombre del ministerio"><input required value={settings.ministry_name} onChange={(event) => setSettings({ ...settings, ministry_name: event.target.value })} className={inputStyles} /></Field><Field label="Logo"><div role="img" aria-label="Vista previa del logo" className="mt-2 h-32 w-32 rounded-2xl border border-white/10 bg-zinc-950 bg-contain bg-center bg-no-repeat" style={{ backgroundImage: `url("${previewUrl}")` }} /><input type="file" accept="image/png,image/jpeg,image/webp,.png,.jpg,.jpeg,.webp" onChange={(event) => selectLogo(event.target.files?.[0] ?? null)} className="mt-3 block w-full text-sm text-zinc-400 file:mr-3 file:min-h-11 file:rounded-xl file:border-0 file:bg-zinc-800 file:px-4 file:font-semibold file:text-white" /></Field></SettingsSection>
    <SettingsSection title="Servicio"><div className="grid gap-4 sm:grid-cols-2"><Field label="Día habitual"><input required value={settings.service_day} onChange={(event) => setSettings({ ...settings, service_day: event.target.value })} className={inputStyles} /></Field><Field label="Hora habitual"><input required value={settings.service_time} onChange={(event) => setSettings({ ...settings, service_time: event.target.value })} className={inputStyles} /></Field></div></SettingsSection>
    <SettingsSection title="Información"><dl className="space-y-2 text-sm"><div className="flex justify-between gap-4"><dt className="text-zinc-500">Nombre de la aplicación</dt><dd className="font-medium text-zinc-200">Gracia Worship</dd></div><div className="flex justify-between gap-4"><dt className="text-zinc-500">Versión actual</dt><dd className="font-medium text-zinc-200">{version}</dd></div></dl></SettingsSection>
    <div><PrimaryButton type="submit" disabled={isSaving || !settings.church_name.trim() || !settings.ministry_name.trim() || !settings.service_day.trim() || !settings.service_time.trim()}>{isSaving ? "Guardando..." : "Guardar cambios"}</PrimaryButton><p role="status" aria-live="polite" className={`mt-3 min-h-5 text-sm ${isError ? "text-rose-400" : "text-emerald-400"}`}>{message}</p></div>
  </form>;
}

function SettingsSection({ children, title }: { children: React.ReactNode; title: string }) { return <section className="rounded-3xl border border-white/[0.07] bg-zinc-900/60 p-5 sm:p-6"><h2 className="text-xl font-semibold text-white">{title}</h2><div className="mt-5 space-y-4">{children}</div></section>; }
function Field({ children, label }: { children: React.ReactNode; label: string }) { return <label className="block text-sm font-semibold text-zinc-300">{label}{children}</label>; }
