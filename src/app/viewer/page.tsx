...
export const metadata: Metadata = { title: "Visor | Gracia Worship" };
  ...
  return (
    <main className="fixed inset-0 z-[60] flex flex-col bg-zinc-950">
      ...
      <div className="min-h-0 flex-1 overflow-hidden bg-zinc-900">
        {source ? <iframe src={`${source}#toolbar=0&navpanes=0&scrollbar=1&view=FitH`} title={`Partitura de ${song.title}`} className="h-full w-full border-0 bg-white" /> : <UnavailableMessage>Partitura no disponible</UnavailableMessage>}
      </div>
    </main>
  );
}
