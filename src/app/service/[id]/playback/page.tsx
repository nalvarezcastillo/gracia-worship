...
export default async function PlaybackPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: songs, error: songError } = songIds.length
    ? await supabase.from("songs").select("id, title, artist, key, bpm, duration, time_signature, cover_url, song_keys(id, key_name, grid_bpm, grid_beats_per_bar, grid_beat_unit, grid_offset_seconds, song_stems(id, song_key_id, name, storage_path, sort_order, mime_type))").in("id", songIds)
    : { data: [], error: null };
