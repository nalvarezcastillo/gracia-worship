...
type PlaybackSong = { artist: string | null; bpm: number | null; cover_url: string | null; duration: string | null; id: string; key: string | null; time_signature: string | null; title: string; song_keys: { id: string; key_name: string; song_sections: { id: string; label: string; section_type: null; song_key_id: string; sort_order: number; start_seconds: number }[]; song_stems: { id: string; mime_type: string | null; name: string; song_key_id: string; sort_order: number; storage_path: string }[] }[] };
...
export default async function PlaybackPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: service }, { data: itemRows, error: itemError }, { data: songs, error: songError }, { data: settings, error: settingsError }] = await Promise.all([
    ...
    supabase.from("songs").select("id, title, artist, key, bpm, duration, time_signature, cover_url, song_keys(id, key_name, song_stems(id, song_key_id, name, storage_path, sort_order, mime_type), song_sections(id, song_key_id, label, section_type, start_seconds, sort_order))"),
  ...
  const entries = operationalEntries.map<PlaybackEntry>((entry) => {
    ...
    return { ...base, artist: entry.song.artist, bpm: entry.song.bpm, coverUrl: entry.song.cover_url || null, effectiveKey: entry.effectiveKey, kind: "song", sections: keyVariant?.song_sections ?? [], stems, timeSignature: entry.song.time_signature };
  });
